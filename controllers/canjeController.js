...
const realizarCanje = async (req, res) => {
  ...
  try {
    ...
    if (usuario.rows[0].puntos_acumulados < prod.costo_puntos) {
      await client.query('ROLLBACK');
      return res.status(400).json({ 
        success: false, 
    ...
    const canje = await client.query(
      `INSERT INTO reciclaje.Canjes 
       ...
       VALUES ($1, $2, $3, $4, 'A') 
       RETURNING *`,
      [prod.id_tienda, id_usuario, id_producto, prod.costo_puntos]
    );
  ...
  try {
    ...
    const verificacion = await db.query(
      `SELECT 
         ...
         u.puntos_acumulados,
         CASE 
           WHEN u.puntos_acumulados >= p.costo_puntos AND p.stock > 0 THEN true 
           ELSE false 
         END as puede_canjear,
         CASE 
           WHEN u.puntos_acumulados < p.costo_puntos THEN 'puntos_insuficientes'
